...
@patch("src.generation.chat.get_openai_client")
@patch("os.environ.get")
def test_generate_answer_refusal(mock_env_get, mock_get_openai_client):
    # Mock environment to have a deployment
    def env_side_effect(key, default=None):
        if key == "AZURE_OPENAI_CHAT_DEPLOYMENT":
            return "dummy-chat"
    ...
    # Mock AzureOpenAI client returning the exact refusal string
    mock_client = MagicMock()
    mock_get_openai_client.return_value = mock_client
    
    mock_response = MagicMock()
    mock_choice = MagicMock()
    mock_choice.message.content = "I could not find this information in the provided documents."
    mock_response.choices = [mock_choice]
    mock_client.chat.completions.create.return_value = mock_response
    ...
    answer, sources = generate_answer("What is the company policy on bringing pets to the office?", chunks)
    
    assert answer == "I could not find this information in the provided documents."
    assert len(sources) == 0  # Sources should be cleared on refusal
